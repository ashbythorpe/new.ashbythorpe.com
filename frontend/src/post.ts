...
async function getComments(): Promise<CommentsResult> {
    return await fetch(`/api/post/${postName()}/comments?page=${page}`).then(
        (x) => x.json(),
    );
}

async function renderPage(page: number) {
    const [{ totalComments, comments }, user] = await Promise.all([
        getComments(),
        getUser(),
    ]);
